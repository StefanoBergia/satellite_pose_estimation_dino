"""
Export HRNet-W32 satellite pose estimation model to ONNX (and optionally TensorRT).

Exports the HRNet-W32 heatmap-based keypoint model for deployment on NVIDIA Jetson Orin.
The ONNX model is the primary deliverable; TensorRT engine must be rebuilt on-device.

Usage:
    python export_hrnet_tensorrt.py --config config_hrnet_w32.yaml --checkpoint outputs_hrnet_w32/converted_model.pth
    python export_hrnet_tensorrt.py --config config_hrnet_w32.yaml --checkpoint outputs_hrnet_w32/converted_model.pth --output_onnx hrnet_w32.onnx --validate
    python export_hrnet_tensorrt.py --config config_hrnet_w32.yaml --checkpoint outputs_hrnet_w32/converted_model.pth --build_trt --fp16
    python export_hrnet_tensorrt.py --config config_hrnet_w32.yaml --checkpoint outputs_hrnet_w32/converted_model.pth --build_trt --int8 --calib_images images/val --calib_labels labels/val
"""
...
class OnnxExportWrapper(torch.nn.Module):
    """Wrapper that converts dict outputs to named tuple-like outputs for ONNX."""
    ...
    def forward(self, pixel_values: torch.Tensor):
        ...
        # Return keypoints and heatmaps as separate named outputs
        return out["keypoints"], out["heatmaps"]


def build_model(config, checkpoint_path, device):
    """Build and load HRNet-W32 model in keypoint_only mode."""
...
def build_trt_engine(onnx_path, trt_path, image_size, fp16, int8, dynamic_batch,
    ...
    if int8:
        ...
        if calib_images and calib_labels:
            cache_path = calib_cache or "hrnet_w32_int8_calib.cache"
...
def main():
    parser = argparse.ArgumentParser(description="Export HRNet-W32 model to ONNX/TensorRT")
    parser.add_argument("--config", type=str, default="config_hrnet_w32.yaml", help="Config YAML path")
    parser.add_argument("--checkpoint", type=str, required=True, help="Model checkpoint path")
    parser.add_argument("--output_onnx", type=str, default="hrnet_w32.onnx", help="Output ONNX path")
    parser.add_argument("--output_trt", type=str, default="hrnet_w32.engine", help="Output TensorRT engine path")
    ...
    imagenet_normalize = config.get("training", {}).get("imagenet_normalize", False)
