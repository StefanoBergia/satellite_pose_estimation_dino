...
python -m domain_adaptation.option4_tta.evaluate_tta \
    --checkpoint "${CHECKPOINT}" --method norm_adapt \
    --no_crop ${EVAL_PARAMS}

echo ""
echo "=== HRNet-W32 TTA: TENT (5 steps) ==="
python -m domain_adaptation.option4_tta.evaluate_tta \
    --checkpoint "${CHECKPOINT}" --method tent --num_steps 5 \
    --no_crop ${EVAL_PARAMS}

echo ""
echo "=== HRNet-W32 TTA: TENT (10 steps) ==="
python -m domain_adaptation.option4_tta.evaluate_tta \
    --checkpoint "${CHECKPOINT}" --method tent --num_steps 10 \
    --no_crop ${EVAL_PARAMS}
